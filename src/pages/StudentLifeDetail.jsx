...
import {
  ArrowLeft, ArrowRight, CheckCircle2, Star, Users, Quote,
  Trophy, BookOpen, Heart
} from 'lucide-react'
import PageHero from '../components/PageHero.jsx'
import Reveal from '../components/Reveal.jsx'
import CtaBanner from '../components/CtaBanner.jsx'
import { findBySlug, getRelated, categoryMeta } from '../data/studentLifeData.js'
    ...
    >
      <div className="relative h-36 overflow-hidden bg-slate-100">
        ...
        <div className="absolute bottom-0 left-0 p-3">
          <span
            className="text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded text-white"
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="bg-white border-b border-slate-100">
        <div className="container-page py-3 flex items-center gap-2 text-xs text-slate-500">
          <Link to="/student-life" className="hover:text-forest transition-colors">
            Student Life
          </Link>
        ...
        <div className="lg:col-span-2 space-y-10">
          ...
          <Reveal>
            <section>
              ...
              <h2 className="section-title mt-1 mb-4">{item.fullName || item.name}</h2>
              ...
              <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {item.highlights.map((h, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-3 p-4 rounded-xl bg-cream/60 border border-slate-100"
                    >
                      <CheckCircle2
                        size={18}
              ...
              <section>
                ...
                <div className="space-y-3">
                  {item.achievements.map((a, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-4 p-4 rounded-xl bg-white border border-slate-100 shadow-sm hover:shadow-card transition-shadow"
                    >
                      <div
                        className="w-9 h-9 rounded-full flex items-center justify-center text-white shrink-0 font-bold text-xs"
                        style={{ backgroundColor: item.accentColor }}
                      >
                        {i + 1}
                      </div>
                  ...
                  {item.galleryImages.map((src, i) => (
                    <div
                      ...
                      className={`relative overflow-hidden rounded-xl bg-slate-100 ${
                        i === 0 ? 'col-span-2 row-span-2 h-52 md:h-full' : 'h-24 md:h-28'
                      }`}
                    >
                      <img
                        src={src}
        ...
        <div className="space-y-6">
          ...
          <Reveal>
            ...
            >
              ...
              <div className="mt-4 pt-4 border-t border-white/20">
                ...
                <p className="text-white font-bold">{item.fullName || item.name}</p>
              </div>
            </div>
          </Reveal>
          ...
          <Reveal>
            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-card">
              <h4 className="font-serif font-bold text-forest text-lg mb-4">Quick Facts</h4>
              <dl className="space-y-3 text-sm">
                ...
                {item.achievements && (
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <dd className="font-bold text-forest">{item.achievements.length}</dd>
                  </div>
                )}
                {item.highlights && (
                  <div className="flex items-start justify-between gap-2">
                    <dt className="text-slate-400 shrink-0">Highlights</dt>
                    <dd className="font-bold text-forest">{item.highlights.length}</dd>
                  </div>
                )}
              </dl>
            </div>
          ...
          <Reveal>
            <div className="bg-cream rounded-2xl p-6 border border-gold/20 text-center">
              <CatIcon size={32} className="text-gold mx-auto mb-3" />
              <h4 className="font-serif font-bold text-forest text-lg mb-2">
                Want to Join?
              </h4>
              <p className="text-xs text-slate-500 leading-relaxed mb-4">
                Speak to the patron or visit the admissions office to find out how to get involved.
              </p>
          ...
          <Reveal>
            <div className="flex flex-col gap-2">
              <Link
                to={`/student-life/${category}`}
                className="flex items-center gap-2 text-sm text-slate-500 hover:text-forest transition-colors"
              >
                <ArrowLeft size={14} /> Back to {catLabel}
              </Link>
              <Link
                to="/student-life"
                className="flex items-center gap-2 text-sm text-slate-500 hover:text-forest transition-colors"
              >
                <ArrowLeft size={14} /> Back to Student Life Hub
              </Link>
            </div>
          </Reveal>
        </div>
          ...
          <div className="container-page">
            <Reveal>
              <div className="flex items-center justify-between mb-6">
                ...
                <Link
                  to={`/student-life/${category}`}
                  className="text-sm font-semibold text-forest hover:text-gold border-b-2 border-forest hover:border-gold transition-all pb-0.5 flex items-center gap-1"
                >
