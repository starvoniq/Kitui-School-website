...
export default function Reveal({
  ...
  const travel = (dir) => {
    switch (dir) {
      case 'left':  return { x: x ?? OFFSET, y: y ?? 0 }
      case 'right': return { x: x ?? -OFFSET, y: y ?? 0 }
      case 'down':  return { x: x ?? 0, y: y ?? -OFFSET }
      case 'none':  return { x: x ?? 0, y: y ?? 0 }
      default:      return { x: x ?? 0, y: y ?? OFFSET }
    }
  }
